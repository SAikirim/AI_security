...
typedef struct _IMAGE_FILE_HEADER {
    ...
    DWORD   TimeDateStamp;		// 프로그램이 빌드되는 시간
    DWORD   PointerToSymbolTable;
    DWORD   NumberOfSymbols;
    WORD    SizeOfOptionalHeader;	// Optional Header 구조체 크기
...
typedef struct _IMAGE_OPTIONAL_HEADER {
    ...
    WORD    Magic;					// 32bit 0x010b	64bit 0x020b
    BYTE    MajorLinkerVersion;
    BYTE    MinorLinkerVersion;
    DWORD   SizeOfCode;				// .text 섹션의 길이(.text는 코드가 존재)
    DWORD   SizeOfInitializedData;
    DWORD   SizeOfUninitializedData;
    DWORD   AddressOfEntryPoint;	// 프로세스의 시작위치 주소 값(RVA)
    DWORD   BaseOfCode;				// 코드의 시작 위치(RVA)
    DWORD   BaseOfData;				// 데이터의 시작 위치 (RVA)

    //
    // NT additional fields.
    //

    DWORD   ImageBase;						// 메모리에서의 기준주소 exe 00400000 dll 10000000
    DWORD   SectionAlignment;				// 메모리에서 섹션의 기본 크기 즉, 메모리에서 섹션의 크기는 이 값의 배수
    DWORD   FileAlignment;					// 파일에서 섹션의 기본 크기 파일에서 세션의 크기는 이 값의 배수
    ...
    DWORD   SizeOfImage;					// 메모리상에서 올라간 이미지의 크기
    DWORD   SizeOfHeaders;					// PE 헤더의 크기
    DWORD   CheckSum;
    WORD    Subsystem;						// 시스템 파일인지 일반파일(gui, 콘솔)인지 구분
    WORD    DllCharacteristics;				// ASLR이 되어 있는지 확인 할 수 있음, .relocation 섹션을 날리거나, DllCharacteristics 값을 40 -> 00
    DWORD   SizeOfStackReserve;
    DWORD   SizeOfStackCommit;
    DWORD   SizeOfHeapReserve;
    DWORD   SizeOfHeapCommit;
    DWORD   LoaderFlags;
    DWORD   NumberOfRvaAndSizes;			// DATA_DIRECTORY의 배열 개수
...
typedef struct _IMAGE_SECTION_HEADER {
    ...
    union {									// union은 둘 중 하나만 씀
            DWORD   PhysicalAddress;
            DWORD   VirtualSize;			// 메모리에서 섹션이 차지하는 크기
    } Misc;
    DWORD   VirtualAddress;					// 메모리에서 섹션의 시작 주소
    DWORD   SizeOfRawData;					// 파일에서 섹션이 차지하는 크기
    DWORD   PointerToRawData;				// !중요! 파일에서 섹션의 시작 위치 !중요!
...
typedef struct _IMAGE_IMPORT_DESCRIPTOR {
    union {
        ...
        DWORD   OriginalFirstThunk;         // RVA to original unbound IAT (PIMAGE_THUNK_DATA), 주로 이걸 사용
    } DUMMYUNIONNAME;
